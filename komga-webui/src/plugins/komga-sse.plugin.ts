...
const vuexModule: Module<any, any> = {
  state: {
    ...
    readyTaskCountByType: {} as { [key: string]: number },
    runningTaskCountByType: {} as { [key: string]: number },
  ...
  mutations: {
    setTaskCount (state, event) {
      ...
      state.readyTaskCountByType = event.readyCountByType || {}
      state.runningTaskCountByType = event.runningCountByType || {}
