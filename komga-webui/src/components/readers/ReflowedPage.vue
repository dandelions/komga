  ...
  methods: {
    ...
    reflowDetectionKey(): string {
      return JSON.stringify({
        ...
        wordBlockTransparencyVersion: 3,
      })
    },
    emitReflowed() {
      this.$emit('reflowed', {
    ...
    transparentizeWordBlockBackground(targetContext: CanvasRenderingContext2D, width: number, height: number) {
      const imageData = targetContext.getImageData(0, 0, width, height)
      const data = imageData.data
      const background = this.estimatedWordBlockBackground(data, width, height)
    ...
    estimatedWordBlockBackground(data: Uint8ClampedArray, width: number, height: number): {r: number, g: number, b: number, luma: number} {
